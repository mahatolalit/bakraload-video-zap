...
class UniversalDownloader:
    ...
    def download_content(self, url, custom_path=None):
        ...
        if not self.is_valid_url(url):
            return {'status': 'error', 'message': 'Invalid or unsupported URL.'}
...
def download():
    ...
    try:
        ...
        if not downloader.is_valid_url(url):
            return jsonify({'status': 'error', 'message': 'Invalid or unsupported URL.'})
...
def bulk_download():
    ...
    try:
        ...
        results = []
        for url in urls:
            if url.strip() and downloader.is_valid_url(url.strip()):
        ...
        return jsonify({'status': 'error', 'message': 'Bulk download error.'})
...
def download_file(filename):
    ...
    try:
        ...
        return jsonify({'error': 'Error processing file download.'}), 500
...
def download_folder(foldername):
    ...
    try:
        # Prevent directory traversal
        safe_foldername = secure_filename(os.path.basename(foldername))
        folder_path = os.path.join(DOWNLOAD_DIR, safe_foldername)
        if not os.path.abspath(folder_path).startswith(os.path.abspath(DOWNLOAD_DIR)):
            abort(403)
        ...
        return jsonify({'error': 'Error processing folder download.'}), 500
...
def clear_downloads():
    ...
    try:
        ...
        return jsonify({'status': 'error', 'message': 'Error clearing downloads.'})
